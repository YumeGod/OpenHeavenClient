...
import com.me.theresa.fontRenderer.font.geom.GeomUtil;
import com.me.theresa.fontRenderer.font.geom.Line;
import com.me.theresa.fontRenderer.font.geom.NeatTriangulator;
import com.me.theresa.fontRenderer.font.geom.OverTriangulator;
import com.me.theresa.fontRenderer.font.geom.Polygon;
import com.me.theresa.fontRenderer.font.geom.Transform;
import com.me.theresa.fontRenderer.font.geom.Triangulator;
...
implements Serializable {
    ...
    public float[] getPoint(int index) {
        this.checkPoints();
        float[] result = new float[]{this.points[index << 1], this.points[(index << 1) + 1]};
        return result;
    }
    ...
    protected void calculateTriangles() {
        ...
        if (this.points.length >= 6) {
            ...
            for (i = 0; i < this.points.length / 2 - 1; ++i) {
                float x1 = this.points[i << 1];
                float y1 = this.points[(i << 1) + 1];
                float x2 = this.points[(i << 1) + 2];
                float y2 = this.points[(i << 1) + 3];
                area += x1 * y2 - y1 * x2;
            }
    ...
    protected final void checkPoints() {
        if (this.pointsDirty) {
            ...
            if (this.points.length > 0) {
                ...
                for (int i = 0; i < this.points.length / 2; ++i) {
                    this.maxX = Math.max(this.points[i << 1], this.maxX);
                    this.maxY = Math.max(this.points[(i << 1) + 1], this.maxY);
                    this.minX = Math.min(this.points[i << 1], this.minX);
                    this.minY = Math.min(this.points[(i << 1) + 1], this.minY);
                }
            }
