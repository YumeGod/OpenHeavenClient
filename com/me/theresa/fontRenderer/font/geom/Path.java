...
import com.me.theresa.fontRenderer.font.geom.Curve;
...
extends Shape {
    ...
    private ArrayList transform(ArrayList pts, Transform t) {
        float[] in = new float[pts.size() << 1];
        float[] out = new float[pts.size() << 1];
        for (int i = 0; i < pts.size(); ++i) {
            in[i << 1] = ((float[])pts.get(i))[0];
            in[(i << 1) + 1] = ((float[])pts.get(i))[1];
        }
        t.transform(in, 0, out, 0, pts.size());
        ArrayList<float[]> outList = new ArrayList<float[]>();
        for (int i = 0; i < pts.size(); ++i) {
            outList.add(new float[]{out[i << 1], out[(i << 1) + 1]});
        }
        return outList;
    }
