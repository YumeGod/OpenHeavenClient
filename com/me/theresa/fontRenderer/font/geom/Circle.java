...
import com.me.theresa.fontRenderer.font.geom.Ellipse;
...
extends Ellipse {
    ...
    public boolean intersects(Shape shape) {
        if (shape instanceof Circle) {
            ...
            float totalRad2 = this.radius + other.radius;
    ...
    public boolean contains(float x, float y) {
        return (x - this.getX()) * (x - this.getX()) + (y - this.getY()) * (y - this.getY()) < this.radius * this.radius;
    }
    ...
    private boolean intersects(Line other) {
        ...
        boolean intersects = circleCenter.copy().sub(closest).lengthSquared() <= this.radius * this.radius;
        return intersects;
    }
}
