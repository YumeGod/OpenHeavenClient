...
public class OpenALMODPlayer {
    ...
    private final byte[] data = new byte[163840];
    private final ByteBuffer bufferData = BufferUtils.createByteBuffer((int)163840);
    private final IntBuffer unqueued = BufferUtils.createIntBuffer((int)1);
    private int source;
    ...
    public boolean stream(int bufferId) {
        ...
        this.bufferData.limit(frames << 2);
        if (reset) {
            if (this.loop) {
                this.ibxm.seek(0);
