...
public class FastTracker2 {
    ...
    public static Module load_xm(byte[] header_60_bytes, DataInput data_input) throws IOException {
        ...
        String tracker_name = FastTracker2.ascii_text(header_60_bytes, 38, 20);
        ...
        for (idx = 1; idx <= num_instruments; ++idx) {
            try {
                Instrument instrument = FastTracker2.read_xm_instrument(data_input, delta_env);
    ...
    private static Instrument read_xm_instrument(DataInput data_input, boolean delta_env) throws IOException {
        ...
        if (num_samples > 0) {
            ...
            for (idx = 0; idx < env_num_points; ++idx) {
                env_tick = FastTracker2.unsigned_short_le(instrument_header, 129 + (idx << 2));
                env_ampl = FastTracker2.unsigned_short_le(instrument_header, 131 + (idx << 2));
            ...
            envelope.set_num_points(env_num_points);
            for (idx = 0; idx < env_num_points; ++idx) {
                env_tick = FastTracker2.unsigned_short_le(instrument_header, 177 + (idx << 2));
                env_ampl = FastTracker2.unsigned_short_le(instrument_header, 179 + (idx << 2));
                envelope.set_point(idx, env_tick, env_ampl, delta_env);
            }
