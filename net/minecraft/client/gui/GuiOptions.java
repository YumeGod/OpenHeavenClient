    ...
    public void initGui() {
        block6: {
            block5: {
                ...
                for (GameSettings.Options gamesettings$options : GuiOptions.field_146440_f) {
                    if (gamesettings$options.getEnumFloat()) {
                        ...
                        guioptionbutton = new GuiOptionButton(gamesettings$options.returnEnumOrdinal(), GuiOptions.width / 2 - 155 + i % 2 * 160, GuiOptions.height / 6 - 12 + 24 * (i >> 1), gamesettings$options, this.game_settings_1.getKeyBinding(gamesettings$options));
                        this.buttonList.add(guioptionbutton);
                    }
                    ++i;
                }
                if (Minecraft.theWorld == null) break block5;
                enumdifficulty = Minecraft.theWorld.getDifficulty();
                ...
                if (!this.mc.isSingleplayer()) ** GOTO lbl-1000
                if (!Minecraft.theWorld.getWorldInfo().isHardcoreModeEnabled()) {
                    ...
                    this.field_175356_r.enabled = this.field_175356_r.func_175230_c() == false;
                    this.field_175357_i.enabled = this.field_175356_r.func_175230_c() == false;
                } else lbl-1000:
                // 2 sources
            ...
            guioptionbutton1 = new GuiOptionButton(GameSettings.Options.REALMS_NOTIFICATIONS.returnEnumOrdinal(), GuiOptions.width / 2 - 155 + i % 2 * 160, GuiOptions.height / 6 - 12 + 24 * (i >> 1), GameSettings.Options.REALMS_NOTIFICATIONS, this.game_settings_1.getKeyBinding(GameSettings.Options.REALMS_NOTIFICATIONS));
        ...
        this.buttonList.add(new GuiButton(114517, GuiOptions.width / 2 + 5, GuiOptions.height / 6 + 72 - 6, 150, 20, "BetterFPS Setting"));
    }
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.enabled) {
            ...
            if (button.id == 109) {
                ...
                objectArray[0] = new ChatComponentTranslation(Minecraft.theWorld.getWorldInfo().getDifficulty().getDifficultyResourceKey(), new Object[0]);
                this.mc.displayGuiScreen(new GuiYesNo(this, new ChatComponentTranslation("difficulty.lock.title", new Object[0]).getFormattedText(), new ChatComponentTranslation("difficulty.lock.question", objectArray).getFormattedText(), 109));
            }
            if (button.id == 110) {
            ...
            if (button.id == 107) {
                this.mc.gameSettings.saveOptions();
                JOptionPane.showMessageDialog(null, "This button option is disabled.");
            }
        }
    }
