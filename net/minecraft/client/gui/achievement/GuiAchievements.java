...
implements IProgressMeter {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if (this.loadingAchievements) {
            ...
            GuiAchievements.drawCenteredString(this.fontRendererObj, lanSearchStates[(int)(Minecraft.getSystemTime() / 150L % (long)lanSearchStates.length)], width / 2, height / 2 + (this.fontRendererObj.FONT_HEIGHT << 1), 0xFFFFFF);
        } else {
    ...
    protected void drawAchievementScreen(int p_146552_1_, int p_146552_2_, float p_146552_3_) {
        ...
        while ((float)l3 * f - (float)j2 < 155.0f) {
            ...
            while ((float)i4 * f1 - (float)i2 < 224.0f) {
                random.setSeed(this.mc.getSession().getPlayerID().hashCode() + k1 + i4 + (l1 + l3 << 4));
                ...
                GuiAchievements guiAchievements = this;
                guiAchievements.mc.getTextureManager().bindTexture(TextureMap.locationBlocksTexture);
                this.drawTexturedModalRect((i4 << 4) - i2, (l3 << 4) - j2, textureatlassprite, 16, 16);
                ++i4;
            }
            ++l3;
        }
        ...
        GuiAchievements guiAchievements = this;
        guiAchievements.mc.getTextureManager().bindTexture(ACHIEVEMENT_BACKGROUND);
        ...
        for (int i6 = 0; i6 < AchievementList.achievementList.size(); ++i6) {
            ...
            GuiAchievements guiAchievements2 = this;
            guiAchievements2.mc.getTextureManager().bindTexture(ACHIEVEMENT_BACKGROUND);
        ...
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        GuiAchievements guiAchievements3 = this;
        guiAchievements3.mc.getTextureManager().bindTexture(ACHIEVEMENT_BACKGROUND);
