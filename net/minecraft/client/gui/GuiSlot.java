...
public abstract class GuiSlot {
    ...
    public void actionPerformed(GuiButton button) {
        if (button.enabled) {
            if (button.id == this.scrollUpButtonID) {
                this.amountScrolled -= (float)((this.slotHeight << 1) / 3);
                this.initialClickY = -2;
                this.bindAmountScrolled();
    ...
    public void drawScreen(int mouseXIn, int mouseYIn, float p_148128_3_) {
        if (this.field_178041_q) {
            ...
            RenderUtil.makeScissorBox(this.left, this.top, this.right, this.bottom);
    ...
    public void handleMouseInput() {
        if (this.isMouseYWithinSlotBounds(this.mouseY)) {
            ...
            if (Mouse.isButtonDown((int)0) && this.enabled) {
    ...
    protected void overlayBackground(int startY, int endY, int startAlpha, int endAlpha) {
        Tessellator tessellator = Tessellator.getInstance();
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        GuiSlot guiSlot = this;
        guiSlot.mc.getTextureManager().bindTexture(Gui.optionsBackground);
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        float f = 32.0f;
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
