...
extends Gui {
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn) {
        ...
        for (NetworkPlayerInfo networkplayerinfo : list) {
            int k = Minecraft.fontRendererObj.getStringWidth(this.getPlayerName(networkplayerinfo));
            i = Math.max(i, k);
        ...
        if (list1 != null) {
            GuiPlayerTabOverlay.drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, k1 + list1.size() * Minecraft.fontRendererObj.FONT_HEIGHT, Integer.MIN_VALUE);
            for (String s3 : list1) {
        ...
        for (int k4 = 0; k4 < l3; ++k4) {
            ...
            if (flag) {
                ...
                GuiPlayerTabOverlay guiPlayerTabOverlay = this;
                guiPlayerTabOverlay.mc.getTextureManager().bindTexture(networkplayerinfo1.getLocationSkin());
                int l2 = 8 + (flag1 ? 8 : 0);
                int i3 = 8 * (flag1 ? -1 : 1);
        ...
        if (list2 != null) {
            k1 = k1 + i4 * 9 + 1;
            GuiPlayerTabOverlay.drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1, k1 + list2.size() * Minecraft.fontRendererObj.FONT_HEIGHT, Integer.MIN_VALUE);
            for (String s4 : list2) {
    ...
    public void drawPing(int p_175245_1_, int p_175245_2_, int p_175245_3_, NetworkPlayerInfo networkPlayerInfoIn) {
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        GuiPlayerTabOverlay guiPlayerTabOverlay = this;
        guiPlayerTabOverlay.mc.getTextureManager().bindTexture(icons);
        boolean i = false;
        int j = 0;
        ...
        zLevel += 100.0f;
        this.drawTexturedModalRect(p_175245_2_ + p_175245_1_ - 11, p_175245_3_, 0, 176 + (j << 3), 10, 8);
        zLevel -= 100.0f;
    }
    ...
    private void drawScoreboardValues(ScoreObjective p_175247_1_, int p_175247_2_, String p_175247_3_, int p_175247_4_, int p_175247_5_, NetworkPlayerInfo p_175247_6_) {
        ...
        if (p_175247_1_.getRenderType() == IScoreObjectiveCriteria.EnumRenderType.HEARTS) {
            ...
            if (j > 0) {
                ...
                if (f > 3.0f) {
                    ...
                    for (int j1 = 0; j1 < j; ++j1) {
                        ...
                        if (flag) {
                            if ((j1 << 1) + 1 < p_175247_6_.func_178860_m()) {
            ...
            String s1 = (Object)((Object)EnumChatFormatting.YELLOW) + String.valueOf(i);
            Minecraft.fontRendererObj.drawStringWithShadow(s1, p_175247_5_ - Minecraft.fontRendererObj.getStringWidth(s1), p_175247_2_, 0xFFFFFF);
        }
    }
