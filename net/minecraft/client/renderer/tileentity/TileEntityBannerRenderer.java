        ...
        if (tileentitybannerrenderer$timedbannertexture == null) {
            if (DESIGNS.size() >= 256) {
                ...
                while (iterator.hasNext()) {
                    ...
                    Minecraft.getTextureManager().deleteTexture(tileentitybannerrenderer$timedbannertexture1.bannerTexture);
                    iterator.remove();
                }
                if (DESIGNS.size() >= 256) {
                    return null;
            ...
            Minecraft.getMinecraft();
            Minecraft.getTextureManager().loadTexture(tileentitybannerrenderer$timedbannertexture.bannerTexture, new LayeredColorMaskTexture(BANNERTEXTURES, list2, list));
