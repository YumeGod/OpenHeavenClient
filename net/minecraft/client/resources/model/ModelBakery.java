...
public class ModelBakery {
    ...
    final Map<ResourceLocation, TextureAtlasSprite> sprites = Maps.newHashMap();
    ...
    private void loadVariantModels() {
        for (Map.Entry<ModelResourceLocation, ModelBlockDefinition.Variants> entry : this.variants.entrySet()) {
            for (ModelBlockDefinition.Variant modelblockdefinition$variant : entry.getValue().getVariants()) {
                ...
                try {
                    ...
                    LOGGER.warn("Unable to load block model: '" + resourcelocation + "' for variant: '" + entry.getKey() + "'", (Throwable)exception);
                }
            }
        }
    }
    ...
    private void bakeBlockModels() {
        ...
        for (Map.Entry<Object, Object> entry : this.itemLocations.entrySet()) {
            ModelBlock modelblock1;
    ...
    protected IBakedModel bakeModel(ModelBlock p_bakeModel_1_, ITransformation p_bakeModel_2_, boolean p_bakeModel_3_) {
        ...
        for (BlockPart blockpart : p_bakeModel_1_.getElements()) {
            for (Map.Entry<EnumFacing, BlockPartFace> entry : blockpart.mapFaces.entrySet()) {
                ...
                boolean flag = true;
                if (Reflector.ForgeHooksClient.exists()) {
                    flag = TRSRTransformation.isInteger();
                }
