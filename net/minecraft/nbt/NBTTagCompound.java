...
extends NBTBase {
    Map<String, NBTBase> tagMap = Maps.newHashMap();

    @Override
    void write(DataOutput output) throws IOException {
        for (Map.Entry<String, NBTBase> entry : this.tagMap.entrySet()) {
            NBTBase nbtbase = entry.getValue();
            NBTTagCompound.writeEntry(entry.getKey(), nbtbase, output);
        }
        output.writeByte(0);
    }
    ...
    public void merge(NBTTagCompound other) {
        for (Map.Entry<String, NBTBase> entry : other.tagMap.entrySet()) {
