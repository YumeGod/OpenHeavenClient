...
implements IChunkProvider {
    ...
    public void setBlocksInChunk(int x, int z, ChunkPrimer primer) {
        this.biomesForGeneration = this.worldObj.getWorldChunkManager().getBiomesForGeneration(this.biomesForGeneration, (x << 2) - 2, (z << 2) - 2, 10, 10);
        this.func_147423_a(x << 2, 0, z << 2);
        for (int i = 0; i < 4; ++i) {
            int j = i * 5;
            int k = (i + 1) * 5;
            for (int l = 0; l < 4; ++l) {
                    ...
                    for (int j2 = 0; j2 < 8; ++j2) {
                        ...
                        for (int k2 = 0; k2 < 4; ++k2) {
                            ...
                            for (int l2 = 0; l2 < 4; ++l2) {
                                ...
                                lvt_45_1_ += d16;
                                if (d > 0.0) {
                                    primer.setBlockState((i << 2) + k2, (i2 << 3) + j2, (l << 2) + l2, Blocks.stone.getDefaultState());
                                    continue;
                                }
                                if ((i2 << 3) + j2 >= this.settings.seaLevel) continue;
                                primer.setBlockState((i << 2) + k2, (i2 << 3) + j2, (l << 2) + l2, this.oceanBlockTmpl.getDefaultState());
                            }
                            d10 += d12;
                            d11 += d13;
                        }
    ...
    public void replaceBlocksForBiome(int x, int z, ChunkPrimer primer, BiomeGenBase[] biomeGens) {
        double d0 = 0.03125;
        this.stoneNoise = this.field_147430_m.func_151599_a(this.stoneNoise, x << 4, z << 4, 16, 16, d0 * 2.0, d0 * 2.0, 1.0);
        for (int i = 0; i < 16; ++i) {
            for (int j = 0; j < 16; ++j) {
                BiomeGenBase biomegenbase = biomeGens[j + (i << 4)];
                biomegenbase.genTerrainBlocks(this.worldObj, this.rand, primer, (x << 4) + i, (z << 4) + j, this.stoneNoise[j + (i << 4)]);
            }
        }
    }
    ...
    public Chunk provideChunk(int x, int z) {
        ...
        this.biomesForGeneration = this.worldObj.getWorldChunkManager().loadBlockGeneratorData(this.biomesForGeneration, x << 4, z << 4, 16, 16);
    ...
    public void populate(IChunkProvider chunkProvider, int x, int z) {
        ...
        long k = (this.rand.nextLong() / 2L << 1) + 1L;
        long l = (this.rand.nextLong() / 2L << 1) + 1L;
