...
implements IChunkProvider {
    ...
    public void func_180516_b(int p_180516_1_, int p_180516_2_, ChunkPrimer p_180516_3_) {
        ...
        double d0 = 0.03125;
        this.slowsandNoise = this.slowsandGravelNoiseGen.generateNoiseOctaves(this.slowsandNoise, p_180516_1_ << 4, p_180516_2_ << 4, 0, 16, 16, 1, d0, d0, 1.0);
        this.gravelNoise = this.slowsandGravelNoiseGen.generateNoiseOctaves(this.gravelNoise, p_180516_1_ << 4, 109, p_180516_2_ << 4, 16, 1, 16, d0, 1.0, d0);
        this.netherrackExclusivityNoise = this.netherrackExculsivityNoiseGen.generateNoiseOctaves(this.netherrackExclusivityNoise, p_180516_1_ << 4, p_180516_2_ << 4, 0, 16, 16, 1, d0 * 2.0, d0 * 2.0, d0 * 2.0);
        for (int j = 0; j < 16; ++j) {
            for (int k = 0; k < 16; ++k) {
                boolean flag = this.slowsandNoise[j + (k << 4)] + this.hellRNG.nextDouble() * 0.2 > 0.0;
                boolean flag1 = this.gravelNoise[j + (k << 4)] + this.hellRNG.nextDouble() * 0.2 > 0.0;
                int l = (int)(this.netherrackExclusivityNoise[j + (k << 4)] / 3.0 + 3.0 + this.hellRNG.nextDouble() * 0.25);
                int i1 = -1;
    ...
    public Chunk provideChunk(int x, int z) {
        ...
        BiomeGenBase[] abiomegenbase = this.worldObj.getWorldChunkManager().loadBlockGeneratorData(null, x << 4, z << 4, 16, 16);
