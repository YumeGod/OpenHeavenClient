...
public class Chunk {
    ...
    private void propagateSkylightOcclusion(int x, int z) {
        this.updateSkylightColumns[x + (z << 4)] = true;
        this.isGapLightingUpdated = true;
    }

    private void recheckGaps(boolean p_150803_1_) {
        this.worldObj.theProfiler.startSection("recheckGaps");
        if (this.worldObj.isAreaLoaded(new BlockPos((this.xPosition << 4) + 8, 0, (this.zPosition << 4) + 8), 16)) {
            for (int i = 0; i < 16; ++i) {
                for (int j = 0; j < 16; ++j) {
                    if (!this.updateSkylightColumns[i + (j << 4)]) continue;
                    this.updateSkylightColumns[i + (j << 4)] = false;
                    int k = this.getHeightValue(i, j);
                    int l = (this.xPosition << 4) + i;
                    int i1 = (this.zPosition << 4) + j;
    ...
    private void relightBlock(int x, int y, int z) {
        ...
        if (j != i) {
            this.worldObj.markBlocksDirtyVertical(x + (this.xPosition << 4), z + (this.zPosition << 4), j, i);
            this.heightMap[z << 4 | x] = j;
            int k = (this.xPosition << 4) + x;
            int l = (this.zPosition << 4) + z;
    ...
    public Block getBlock(final int x, final int y, final int z) {
        try {
            ...
            crashreportcategory.addCrashSectionCallable("Location", new Callable<String>(){
                ...
                @Override
                public String call() {
                    return CrashReportCategory.getCoordinateInfo(new BlockPos((Chunk.this.xPosition << 4) + x, y, (Chunk.this.zPosition << 4) + z));
                }
            });
