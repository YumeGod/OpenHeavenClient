...
extends Entity {
    ...
    public boolean isUp;
    public double AuraHitAnimationsSetup;
    public int hurtTime;
    ...
    protected void damageEntity(DamageSource damageSrc, float damageAmount) {
        if (!this.isEntityInvulnerable(damageSrc)) {
            ...
            if (damageAmount != 0.0f) {
                ...
                this._combatTracker.trackDamage(damageSrc, f1, damageAmount);
    ...
    private int getArmSwingAnimationEnd() {
        if (this.isPotionActive(Potion.digSpeed)) {
            if (Client.instance.getModuleManager().getModuleByClass(Animations.class).isEnabled()) {
                return 6 + ((int)(6.0 - 6.0 * (Double)Animations.speed.getValue()) << 1);
            }
        ...
        return this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier() << 1) : 6;
    }
        ...
        if (this.isSprinting()) {
            ...
            if (this == Minecraft.thePlayer) {
                ...
                if (Minecraft.thePlayer.movementYaw != null) {
                    Minecraft.getMinecraft();
                    f = Minecraft.thePlayer.movementYaw.floatValue() * ((float)Math.PI / 180);
                }
            }
    ...
    protected void jumpOther() {
        this.motionY = this.getJumpUpwardsMotion();
        if (this.isPotionActive(Potion.jump)) {
            this.motionY += (double)((float)(this.getActivePotionEffect(Potion.jump).getAmplifier() + 1) * 0.1f);
        }
        if (this.isSprinting()) {
            float f = this.rotationYawHead * ((float)Math.PI / 180);
            this.motionX -= (double)(MathHelper.sin(f) * 0.2f);
            this.motionZ += (double)(MathHelper.cos(f) * 0.2f);
        }
        this.isAirBorne = true;
    ...
    public void onLivingUpdate() {
        ...
        if (this.isJumping) {
            if (this.isInWater()) {
                ...
                this.jump();
                this.jumpTicks = Client.instance.getModuleManager().getModuleByClass(NoJumpDelay.class).isEnabled() ? 0 : 10;
            }
        } else {
            this.jumpTicks = 0;
        }
    ...
    public Vec3 getLook(float partialTicks) {
        if (Client.instance.getModuleManager().getModuleByClass(MouseDelayFix.class).isEnabled() && this instanceof EntityPlayerSP) {
