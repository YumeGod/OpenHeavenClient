...
implements ICommandSender {
    ...
    protected void setSize(float width, float height) {
        if (width != this.width || height != this.height) {
            ...
            this.width = width;
            this.height = height;
            this.boundingBox = new AxisAlignedBB(this.boundingBox.minX, this.boundingBox.minY, this.boundingBox.minZ, this.boundingBox.minX + (double)this.width, this.boundingBox.minY + (double)this.height, this.boundingBox.minZ + (double)this.width);
    ...
    public boolean isOffsetPositionInLiquid(double x, double y, double z) {
        AxisAlignedBB axisalignedbb = this.boundingBox.offset(x, y, z);
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            double d4 = y;
            double d5 = z;
            boolean bl = flag = this.onGround && this.isSneaking() && this instanceof EntityPlayer || Client.instance.getModuleManager().getModuleByClass(Scaffold.class).isEnabled() && (Boolean)Scaffold.safe.getValue() != false && this instanceof EntityPlayer && this.onGround;
            if (!(!flag || Client.instance.getModuleManager().getModuleByClass(Scaffold.class).isEnabled() && Keyboard.isKeyDown((int)42) && ((Boolean)Scaffold.down.getValue()).booleanValue())) {
                double d6 = 0.05;
                ...
                while (x != 0.0 && z != 0.0 && this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.offset(x, -1.0, z)).isEmpty()) {
                    x = x < d6 && x >= -d6 ? 0.0 : (x > 0.0 ? (x -= d6) : (x += d6));
                    d3 = x;
                    z = z < d6 && z >= -d6 ? 0.0 : (z > 0.0 ? (z -= d6) : (z += d6));
                    d5 = z;
            ...
            for (AxisAlignedBB axisAlignedBB : list1) {
                y = axisAlignedBB.calculateYOffset(this.boundingBox, y);
            }
            this.boundingBox = this.boundingBox.offset(0.0, y, 0.0);
            boolean flag1 = this.onGround || d4 != y && d4 < 0.0;
            for (AxisAlignedBB axisalignedbb2 : list1) {
                x = axisalignedbb2.calculateXOffset(this.boundingBox, x);
            }
            this.boundingBox = this.boundingBox.offset(x, 0.0, 0.0);
            for (AxisAlignedBB axisalignedbb13 : list1) {
                z = axisalignedbb13.calculateZOffset(this.boundingBox, z);
            }
            this.boundingBox = this.boundingBox.offset(0.0, 0.0, z);
            if (this.stepHeight > 0.0f && flag1 && (d3 != x || d5 != z)) {
                double d = x;
                double d7 = y;
                double d8 = z;
                AxisAlignedBB axisalignedbb3 = this.boundingBox;
                this.boundingBox = axisalignedbb;
                y = this.stepHeight;
                List<AxisAlignedBB> list = this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.addCoord(d3, y, d5));
                AxisAlignedBB axisalignedbb4 = this.boundingBox;
                ...
                AxisAlignedBB axisalignedbb14 = this.boundingBox;
                double d17 = y;
                ...
                if (d20 > d10) {
                    ...
                    z = d16;
                    y = -d9;
                    this.boundingBox = axisalignedbb4;
                } else {
                    x = d18;
                    z = d19;
                    y = -d17;
                    this.boundingBox = axisalignedbb14;
                }
                for (AxisAlignedBB axisalignedbb12 : list) {
                    y = axisalignedbb12.calculateYOffset(this.boundingBox, y);
                }
                this.boundingBox = this.boundingBox.offset(0.0, y, 0.0);
                if (d * d + d8 * d8 >= x * x + z * z) {
                    ...
                    y = d7;
                    z = d8;
                    this.boundingBox = axisalignedbb3;
                } else if (this.onGround && this == Minecraft.thePlayer) {
                    EventManager.call(new EventStep(this.stepHeight));
                }
            }
            this.worldObj.theProfiler.endSection();
            ...
            if (this.worldObj.isFlammableWithin(this.boundingBox.contract(0.001, 0.001, 0.001))) {
    ...
    protected void doBlockCollisions() {
        BlockPos blockpos = new BlockPos(this.boundingBox.minX + 0.001, this.boundingBox.minY + 0.001, this.boundingBox.minZ + 0.001);
        BlockPos blockpos1 = new BlockPos(this.boundingBox.maxX - 0.001, this.boundingBox.maxY - 0.001, this.boundingBox.maxZ - 0.001);
    ...
    protected void createRunningParticles() {
        ...
        if (block.getRenderType() != -1) {
            this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + ((double)this.rand.nextFloat() - 0.5) * (double)this.width, this.boundingBox.minY + 0.1, this.posZ + ((double)this.rand.nextFloat() - 0.5) * (double)this.width, -this.motionX * 4.0, 1.5, -this.motionZ * 4.0, Block.getStateId(iblockstate));
        }
    }
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (this == Minecraft.thePlayer) {
            StrafeEvent strafeEvent = new StrafeEvent(forward, strafe, friction);
        ...
        if ((f = strafe * strafe + forward * forward) >= 1.0E-4f) {
            ...
            if (this == Minecraft.thePlayer) {
                if (Minecraft.thePlayer.movementYaw != null) {
                    f1 = MathHelper.sin(Minecraft.thePlayer.movementYaw.floatValue() * (float)Math.PI / 180.0f);
                    f2 = MathHelper.cos(Minecraft.thePlayer.movementYaw.floatValue() * (float)Math.PI / 180.0f);
                }
            }
    ...
    public void writeToNBT(NBTTagCompound tagCompund) {
        try {
            ...
            tagCompund.setLong("UUIDMost", this.entityUniqueID.getMostSignificantBits());
            tagCompund.setLong("UUIDLeast", this.entityUniqueID.getLeastSignificantBits());
    ...
    public void setPositionAndRotation2(double x, double y, double z, float yaw, float pitch, int posRotationIncrements, boolean p_180426_10_) {
        ...
        this.setRotation(yaw, pitch);
        List<AxisAlignedBB> list = this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.contract(0.03125, 0.0, 0.03125));
        if (!list.isEmpty()) {
            double d0 = 0.0;
            ...
            this.setPosition(x, y += d0 - this.boundingBox.minY, z);
        }
    }

    public float getCollisionBorderSize() {
        EventEntityBorderSize e = new EventEntityBorderSize(0.1f);
        EventManager.call(e);
        return e.getBorderSize();
    }
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        ...
        List<AxisAlignedBB> list = this.worldObj.getCollisionBoxes(this.boundingBox);
    ...
    public IChatComponent getDisplayName() {
        ...
        chatcomponenttext.getChatStyle().setInsertion(this.entityUniqueID.toString());
    ...
    protected HoverEvent getHoverEvent() {
        NBTTagCompound nbttagcompound = new NBTTagCompound();
        String s = EntityList.getEntityString(this);
        nbttagcompound.setString("id", this.entityUniqueID.toString());
        if (s != null) {
    ...
    public void setCommandStats(Entity entityIn) {
        this.cmdResultStats.addAllStats(entityIn.cmdResultStats);
    }
    ...
    public double getDistance2D(Entity entityIn) {
        double x = Math.abs(entityIn.posX - this.posX);
        double z = Math.abs(entityIn.posZ - this.posZ);
