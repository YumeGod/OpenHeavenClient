...
implements ICrafting {
    ...
    public EntityPlayerMP(MinecraftServer server, WorldServer worldIn, GameProfile profile, ItemInWorldManager interactionManager) {
        ...
        if (!worldIn.provider.getHasNoSky() && worldIn.getWorldInfo().getGameType() != WorldSettings.GameType.ADVENTURE) {
            ...
            blockpos = worldIn.getTopSolidOrLiquidBlock(blockpos.add(this.rand.nextInt(i << 1) - i, 0, this.rand.nextInt(i << 1) - i));
        }
        ...
        if (!this.loadedChunks.isEmpty()) {
            ...
            while (iterator1.hasNext() && list.size() < 10) {
                ...
                if (chunkcoordintpair != null) {
                    ...
                    list.add(chunk);
                    list1.addAll(((WorldServer)this.worldObj).getTileEntitiesIn(chunkcoordintpair.chunkXPos << 4, 0, chunkcoordintpair.chunkZPos << 4, (chunkcoordintpair.chunkXPos << 4) + 16, 256, (chunkcoordintpair.chunkZPos << 4) + 16));
                    iterator1.remove();
                    continue;
                }
                iterator1.remove();
    ...
    public void onUpdateEntity() {
        try {
            ...
            if (this.ticksExisted % 20 * 5 == 0 && !this.statsFile.hasAchievementUnlocked(AchievementList.exploreAllBiomes)) {
    ...
    protected void updateBiomesExplored() {
        ...
        JsonSerializableSet jsonserializableset = (JsonSerializableSet)this.statsFile.func_150870_b(AchievementList.exploreAllBiomes);
        if (jsonserializableset == null) {
            jsonserializableset = this.statsFile.func_150872_a(AchievementList.exploreAllBiomes, new JsonSerializableSet());
        }
        jsonserializableset.add(s);
        if (this.statsFile.canUnlockAchievement(AchievementList.exploreAllBiomes) && jsonserializableset.size() >= BiomeGenBase.explorationBiomesList.size()) {
