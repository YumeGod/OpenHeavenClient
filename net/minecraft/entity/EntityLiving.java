...
extends EntityLivingBase {
    ...
    public void readEntityFromNBT(NBTTagCompound tagCompund) {
        ...
        if (tagCompund.hasKey("CanPickUpLoot", 1)) {
            this.canPickUpLoot = tagCompund.getBoolean("CanPickUpLoot");
        }
    ...
    public int getMaxFallHeight() {
        ...
        if ((i -= 3 - this.worldObj.getDifficulty().getDifficultyId() << 2) < 0) {
            i = 0;
        }
        return i + 3;
    }
    ...
    public final boolean interactFirst(EntityPlayer playerIn) {
        if (this.isLeashed && this.leashedToEntity == playerIn) {
