...
IPlayerUsage {
    ...
    protected void convertMapIfNeeded(String worldNameIn) {
        if (this.anvilConverterForAnvilFile.isOldMapFormat(worldNameIn)) {
            logger.info("Converting map!");
            this.userMessage = "menu.convertingLevel";
            this.anvilConverterForAnvilFile.convertMapFormat(worldNameIn, new IProgressUpdate(){
    ...
    protected void loadAllWorlds(String saveName, String worldNameIn, long seed, WorldType type, String worldNameIn2) {
        ...
        this.setResourcePackFromWorld(this.folderName, isavehandler);
    ...
    protected void initialWorldChunkLoad() {
        ...
        for (int l1 = -192; l1 <= 192 && this.serverRunning; l1 += 16) {
            for (int i2 = -192; i2 <= 192 && this.serverRunning; i2 += 16) {
    ...
    public void stopServer() {
        if (!this.worldIsBeingDeleted) {
            ...
            if (this.networkSystem != null) {
                this.networkSystem.terminateEndpoints();
            }
    ...
    public void deleteWorldAndStopServer() {
        ...
        this.anvilConverterForAnvilFile.flushCache();
        for (int i = 0; i < this.worldServers.length; ++i) {
            WorldServer worldserver = this.worldServers[i];
            if (worldserver == null) continue;
            worldserver.flush();
        }
        this.anvilConverterForAnvilFile.deleteWorldDirectory(this.worldServers[0].getSaveHandler().getWorldDirectoryName());
    ...
    public void setGameType(WorldSettings.GameType gameMode) {
        for (int i = 0; i < this.worldServers.length; ++i) {
            MinecraftServer.mcServer.worldServers[i].getWorldInfo().setGameType(gameMode);
        }
    }
    ...
    public boolean sendCommandFeedback() {
        return MinecraftServer.mcServer.worldServers[0].getGameRules().getBoolean("sendCommandFeedback");
    }
    ...
    public <V> ListenableFuture<V> callFromMainThread(Callable<V> callable) {
        ...
        if (!this.isCallingFromMinecraftThread() && !this.serverStopped) {
