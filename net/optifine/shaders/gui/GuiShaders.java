...
extends GuiScreenOF {
    ...
    private void actionPerformed(GuiButton button, boolean rightClick) {
        ...
        switch (guibuttonenumshaderoption.getEnumShaderOption()) {
            ...
            case RENDER_RES_MUL: {
                Shaders.configRenderResMul = this.getNextValue(Shaders.configRenderResMul, QUALITY_MULTIPLIERS, 1.0f, !rightClick, this.hasShiftDown());
                Shaders.uninit();
                Shaders.scheduleResize();
                break;
            }
            case SHADOW_RES_MUL: {
                Shaders.configShadowResMul = this.getNextValue(Shaders.configShadowResMul, QUALITY_MULTIPLIERS, 1.0f, !rightClick, this.hasShiftDown());
                Shaders.uninit();
                Shaders.scheduleResizeShadow();
                break;
            }
            case HAND_DEPTH_MUL: {
                Shaders.configHandDepthMul = this.getNextValue(Shaders.configHandDepthMul, HAND_DEPTH_VALUES, 0.125f, !rightClick, this.hasShiftDown());
                Shaders.uninit();
                break;
            }
            case OLD_HAND_LIGHT: {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        GuiShaders.drawCenteredString(this.fontRendererObj, this.screenTitle + " ", width / 2, 15, 0xFFFFFF);
        ...
        if (i < width - 5) {
            GuiShaders.drawCenteredString(this.fontRendererObj, s, width / 2, height - 40, 0x808080);
        } else {
            GuiShaders.drawString(this.fontRendererObj, s, 5, height - 40, 0x808080);
        }
    ...
    public void drawCenteredString(String text, int x, int y, int color) {
        GuiShaders.drawCenteredString(this.fontRendererObj, text, x, y, color);
    }
