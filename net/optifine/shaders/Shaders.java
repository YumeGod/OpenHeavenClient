...
public class Shaders {
    ...
    private static final ProgramStack programStack;
    ...
    public static IntBuffer nextIntBuffer(int size) {
        ByteBuffer bytebuffer = bigBuffer;
        int i = bytebuffer.limit();
        bytebuffer.position(i).limit(i + (size << 2));
    ...
    private static ICustomTexture loadCustomTextureRaw(int textureUnit, String line, String path, TextureType type, InternalFormat internalFormat, int width, int height, int depth, PixelFormat pixelFormat, PixelType pixelType) {
        try {
            ...
            bytebuffer.flip();
            TextureMetadataSection texturemetadatasection = SimpleShaderTexture.loadTextureMetadataSection(s, new TextureMetadataSection(true, true, new ArrayList<Integer>()));
    ...
    public static void init() {
        ...
        if (!isShaderPackInitialized) {
            ...
            dfbColorTextures.position(0).limit(usedColorBuffers << 1);
    ...
    public static void useProgram(Program program) {
        ...
        if (activeProgram != program) {
            ...
            if (i != 0) {
                ...
                ItemStack itemstack = Minecraft.thePlayer != null ? Minecraft.thePlayer.getHeldItem() : null;
                ...
                Shaders.setProgramUniform1f(uniform_far, Shaders.mc.gameSettings.renderDistanceChunks << 4);
    ...
    private static void resize() {
        renderDisplayWidth = Minecraft.displayWidth;
        renderDisplayHeight = Minecraft.displayHeight;
    ...
    public static void beginRender(Minecraft minecraft, float partialTicks, long finishTimeNano) {
        block19: {
            block18: {
                block17: {
                    ...
                    if (!isShaderPackInitialized) {
                        try {
                            ...
                            if (!Config.normalize(illegalstateexception.getMessage()).equals("Function is not supported")) break block17;
                ...
                if (Minecraft.displayWidth != renderDisplayWidth) break block18;
                if (Minecraft.displayHeight == renderDisplayHeight) break block19;
            }
            Shaders.resize();
        }
        if (needResizeShadow) {
        ...
        if (entity != null) {
            ...
            if (Minecraft.thePlayer != null) {
                ...
                if (Minecraft.thePlayer.isPotionActive(Potion.nightVision)) {
                    nightVision = Config.getMinecraft().entityRenderer.getNightVisionBrightness(Minecraft.thePlayer, partialTicks);
                }
                blindness = 0.0f;
                if (Minecraft.thePlayer.isPotionActive(Potion.blindness)) {
                    int i = Minecraft.thePlayer.getActivePotionEffect(Potion.blindness).getDuration();
    ...
    private static void renderFinal() {
        ...
        GL11.glViewport((int)0, (int)0, (int)Minecraft.displayWidth, (int)Minecraft.displayHeight);
    ...
    public static void drawHorizon() {
        ...
        float f = Shaders.mc.gameSettings.renderDistanceChunks << 4;
    ...
    public static int getEntityData() {
        return entityData[entityDataIndex << 1];
    }

    public static int getEntityData2() {
        return entityData[(entityDataIndex << 1) + 1];
    }

    public static int setEntityData1(int data1) {
        Shaders.entityData[Shaders.entityDataIndex << 1] = entityData[entityDataIndex << 1] & 0xFFFF | data1 << 16;
        return data1;
    }

    public static int setEntityData2(int data2) {
        Shaders.entityData[(Shaders.entityDataIndex << 1) + 1] = entityData[(entityDataIndex << 1) + 1] & 0xFFFF0000 | data2 & 0xFFFF;
        return data2;
    }

    public static void pushEntity(int data0, int data1) {
        Shaders.entityData[++Shaders.entityDataIndex << 1] = data0 & 0xFFFF | data1 << 16;
        Shaders.entityData[(Shaders.entityDataIndex << 1) + 1] = 0;
    }

    public static void pushEntity(int data0) {
        Shaders.entityData[++Shaders.entityDataIndex << 1] = data0 & 0xFFFF;
        Shaders.entityData[(Shaders.entityDataIndex << 1) + 1] = 0;
    }

    public static void pushEntity(Block block) {
        int i = block.getRenderType();
        Shaders.entityData[++Shaders.entityDataIndex << 1] = Block.blockRegistry.getIDForObject(block) & 0xFFFF | i << 16;
        Shaders.entityData[(Shaders.entityDataIndex << 1) + 1] = 0;
    }

    public static void popEntity() {
        Shaders.entityData[Shaders.entityDataIndex << 1] = 0;
        Shaders.entityData[(Shaders.entityDataIndex << 1) + 1] = 0;
        --entityDataIndex;
    }
    ...
    public static void nextAntialiasingLevel(boolean forward) {
        ...
        configAntialiasingLevel = configAntialiasingLevel / 2 << 1;
    ...
    static {
        ...
        bigBufferSize = 285 + 8 * ProgramCount << 2;
